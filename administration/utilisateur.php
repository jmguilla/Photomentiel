...
<?php
	foreach($assocs as $assoc){
		...
		echo '<tr><td>#' . $user->getUtilisateurID() . ' - </td><td>' . $user->getAdresse()->getPrenom() . ' ' . $user->getAdresse()->getNom() . '</td><td> - ' . $user->getEmail() . ' </td><td><form action="dispatcher.php" method="post"><input type="hidden" name="id" value="' . $aid . '"/><input type="hidden" name="action" value="activer_utilisateur"/><input type="submit" value="activer"/></form></td><td><form action="dispatcher.php" method="post"><input type="hidden" name="id" value="' . $user->getUtilisateurID() . '"/><input type="hidden" name="action" value="renvoyer_email_confirmation"/><input type="hidden" name="aid" value="' . $aid . '"/><input type="submit" value="renvoyer email"/></form></td><td><form action="dispatcher.php" method="post"><input type="hidden" name="id" value="' . $user->getUtilisateurID() . '"/><input type="hidden" name="action" value="reinitialiser_mdp"/><input type="submit" value="réinitialiser mdp"/></form></td></tr>';
	}
?>
</table>
<?php
